...
const Requests = () => {
    ...
    return (
        <Box sx={{ backgroundColor: '#d3d3d329', px: { xs: 1 }, pt:4, pb:2 }}>
            <Typography variant="h4" align="center" className="main-header" sx={{mb:1}}>
                Requests
            </Typography>
            {
                isLoading ? <>
                                ...
                                >
                                    {
                                        requests?.slice(0, 10).map((request) => {
                                            return <SwiperSlide key={request.id} style={{ marginRight: 0 }}>
                                                <Box sx={{ mx: 2 }}>
